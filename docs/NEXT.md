...
本轮明确不实现导入、云同步、遥测、课程、复习系统或逐条删除。后续产品功能继续依据发布后的真实使用反馈单独规划并等待确认。
...
- 使用当前 DSH 0.1.1-rc.2 和真实模型重新录制完整学习循环 GIF；记录插件提交、DSH 提交和素材哈希。
...
10. **测试伴随**：相关单元/集成测试、keyless assembled Web snapshot、真实 DSH 0.1.1-rc.2 模型流程和 GIF 在同一变更中通过并可追溯。
