...
同时验证 npm rc.1 与官方最新源码 alpha.1：两个源码版本分别通过类型检查、80 项单元/集成测试、7 个真实 Web 场景及 3 个 M6 场景；CI 新增固定提交的双版本组装矩阵。安装渠道和可复现命令见 [COMPATIBILITY.md](./COMPATIBILITY.md)，新增回归证据见 [ACCEPTANCE.md](./ACCEPTANCE.md)。现有 rc.1 Demo 保持其原版本标注。
