...
发布包安装基线为 DSH `0.1.2-rc.1`。本 checkout 同时兼容最新源码发布 `0.1.3-alpha.1`；截至 2026-09-05，该版本 CLI/API 包尚未发布到 npm。精确源码版本与本地安装方式见[兼容约定](docs/COMPATIBILITY.md)。
...
> Explain 最新标签版本仍是 `v0.2.0`，`v0.3.0` 尚未发布；从 GitHub 安装会使用当前已经适配 rc.1 的 `main` 分支。
...
| 检查 | 当前结果 |
|---|---|
| DSH 兼容 | `0.1.2-rc.1` 发布包；`0.1.2-rc.1` 与 `0.1.3-alpha.1` 组装源码 |
| 单元与集成 | 两个源码版本各 80 项测试 |
| DSH Web 组装验收 | 两个源码版本各 7 个场景 |
...
默认开发安装使用已发布的 `0.1.2-rc.1` API 包。组装 Web 测试支持已构建的 DSH `0.1.2-rc.1` 或 `0.1.3-alpha.1` 源码 checkout；现有 Demo 录制仍使用 rc.1：
