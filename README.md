...
<p align="center">
  <img alt="DSH 0.1.3-alpha.2" src="https://img.shields.io/badge/DSH-0.1.3--alpha.2-4c8bf5">
...
The current development and installation baseline is DSH `0.1.3-alpha.2`, available on npm as of 2026-09-08. npm `latest` / `next` still point to `0.1.2-rc.1`, which remains supported alongside `0.1.3-alpha.1`. Pin the version explicitly; see the [compatibility contract](docs/COMPATIBILITY.md) for exact source revisions and verification.
...
> The latest tagged Explain release is `v0.2.0`; `v0.3.0` has not been published. Installing from GitHub uses the current `main` branch with rc.1, alpha.1, and alpha.2 compatibility.
...
|---|---|
| DSH compatibility | `0.1.3-alpha.2` published packages; `0.1.2-rc.1`, `0.1.3-alpha.1`, and `0.1.3-alpha.2` assembled source |
...
The default development install uses published `0.1.3-alpha.2` API packages. Assembled-Web tests accept a built DSH `0.1.2-rc.1`, `0.1.3-alpha.1`, or `0.1.3-alpha.2` checkout. The existing demo recording remains on rc.1:
