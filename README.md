...
<p align="center">
  <img alt="DSH 0.1.7-alpha.2" src="https://img.shields.io/badge/DSH-0.1.7--alpha.2-4c8bf5">
...
DSH `0.1.7-alpha.2` and npm next `0.1.5-rc.3` support are available on `main` and have not been tagged as a new Explain release. They preserve the ten previously verified hosts. The latest Explain release, `v0.3.1`, supports DSH through `0.1.6-alpha.2`. See the [compatibility contract](https://github.com/yuezengwu/dsh-explain/blob/main/docs/COMPATIBILITY.md) for exact source revisions and evidence.
...
For the existing tagged release, keep DSH `0.1.6-alpha.2` and install `github:yuezengwu/dsh-explain#v0.3.1`. As checked on 2026-09-23, npm `alpha` is `0.1.7-alpha.2`, `latest` is `0.1.5-rc.2`, and `next` is `0.1.5-rc.3`. The next channel now has an official source tag and is in the verified matrix. Use `0.1.5-rc.3` in both commands above to install that host.
...
|---|---|
| DSH compatibility | `0.1.7-alpha.2` published packages; `0.1.2-rc.1`, `0.1.3-alpha.1`, `0.1.3-alpha.2`, `0.1.5-alpha.1`, `0.1.5-alpha.2`, `0.1.5-rc.1`, `0.1.5-rc.2`, `0.1.5-rc.3`, `0.1.6-alpha.1`, `0.1.6-alpha.2`, `0.1.7-alpha.1`, `0.1.7-alpha.2` assembled source |
...
| Explain-owned shortcuts | 3 M6 scenarios; 0.1.6-alpha.2 and both 0.1.7 alphas also exercise live disable/re-enable |
...
The default development install uses published `0.1.7-alpha.2` API packages, with a version-scoped repair for their [missing store runtime dependencies](https://github.com/yuezengwu/dsh-explain/blob/main/docs/COMPATIBILITY.md#发布包依赖修复). Assembled-Web tests accept a built DSH checkout for any of the twelve versions above. The existing demo recording remains on `0.1.2-rc.1`:
